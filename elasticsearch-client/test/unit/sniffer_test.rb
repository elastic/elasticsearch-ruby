...
class Elasticsearch::Client::Transport::SnifferTest < Test::Unit::TestCase
  ...
  def __nodes_info(json)
    Elasticsearch::Client::Transport::Response.new 200, MultiJson.load(json)
  end
  ...
  context "Sniffer" do
    ...
    should "raise error on timeout" do
      @transport.expects(:perform_request).raises(Elasticsearch::Client::Transport::SnifferTimeoutError)

      # TODO: Try to inject sleep into `perform_request` or make this test less ridiculous anyhow...
      assert_raise Elasticsearch::Client::Transport::SnifferTimeoutError do
        @sniffer.hosts
      end
    end
