    ...
    module Formatters
      ...
      class HtmlPrinter
        ...
        def print_example_group_start(group_id, description, number_of_parents)
          ...
          @output.puts "  <details>"
          @output.puts "  <summary id=\"example_group_#{group_id}\" class=\"passed\">#{h(description)}</summary>"
        end
        ...
        def print_example_failed(pending_fixed, description, run_time, failure_id,
          ...
          if exception
            @output.puts "        <div class=\"message\"><code>#{h(exception[:message])}</code></div>"
            @output.puts "        <div class=\"backtrace\"><code>#{h exception[:backtrace]}</code></div>"
          end
