...
`BulkHelper` also provides a helper to ingest data straight from a JSON file. You can pass in a file path, and th helper will parse and ingest the documents in the file:
...
In cases where the array of data you want to ingest is not necessarily in the root of the JSON file, you can pass in the keys to access the data, for example given the following JSON file:
...
You would write the following Ruby code to ingest it:
